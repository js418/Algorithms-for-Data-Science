# 590D Algorithm for Data Science
# Jie Song, Buqin Wang, Swetal Bhatt
# Coding Assignment 1
# Question 2
# K-means algorithm


import time
import random
import math
from math import factorial

K = 5

def distance(x,y):
    d = 0
    for i in range(13):
        d += pow(x[i]-y[i],2)
    return d

def center_(a):
    c = []
    n = len(a)
    for i in range(13):
        b = 0
        for j in range(n):
            b += a[j][i]
        c.append(round(b/n,2))
    return c

def com(n, r):
    if n < r:
        return 0
    else:
        return factorial(n) // factorial(r) // factorial(n-r)

def main():
    
    start = time.time()

    print 'Given K =', K

    # store all the datas as points in a list
    f = open('wine.data.txt','r')
    points = []
    for line in f:
        data = line.strip().split(',')
        del data[0]
        for i in range(13):
            data[i] = float(data[i])
        points.append(data)
    f.close
    N = len(points)

    # randomly pick K centers from all points
    c_new = []
    c = random.sample(range(N),K)
    for a in c:
        c_new.append(points[a])

    # Recompute the new cluster centers until the new centers are not changed
    while True:
        c_old = c_new
        clusters = [ [] for k in range(K)]
        c_id = [ [] for k in range(K)]
        # compute the distance between each point and centers
        for i in range(N):
            d = distance(points[i], c_old[0])
            label = 0
            # Assign each point to its nearest cluster center
            for j in range(K):
                if distance(points[i],c_old[j]) < d:
                    d = distance(points[i],c_old[j])
                    label = j
            clusters[label].append(points[i])
            c_id[label].append(i)
        # compute new centers
        for k in range(K):
            c_new[k] = center_(clusters[k])
        if c_old == c_new:
            break

    print 'The centers of', K,'clusters by K-means are:', c_new
    # print 'the cluster of points\' id is:', c_id

    
    for i in range(K):
        print 'Cluster', i, 'with', len(clusters[i]), 'points in it'

    g_id = [ list(i for i in range(59)),list(j for j in range(59,130)),list(k for k in range(130,178))]

    c_in = [ [] for k in range(K)]
    pos_agree = 0
    for i in range(K):
        for j in range(3):
            inter = set(c_id[i]).intersection(g_id[j])
            c_in[i].append(len(inter))
            pos_agree += com(c_in[i][j],2)
    print c_in
    
    neg_agree = 0
    for i in range(K-1):
        for j in range(i+1,K):
            neg_agree += c_in[i][0]*c_in[j][1] + c_in[i][0]*c_in[j][2]
            neg_agree += c_in[i][1]*c_in[j][0] + c_in[i][0]*c_in[j][2]
            neg_agree += c_in[i][2]*c_in[j][0] + c_in[i][0]*c_in[j][1]

    total_agree = pos_agree + neg_agree
    print pos_agree
    print neg_agree
    print 'The total agreement is:', total_agree
        
        
    stop = time.time()

    print 'running time is', stop - start, 'seconds'
    
main()
